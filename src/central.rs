...
async fn main(spawner: Spawner) {
    ...
    // USB driver — central enumerates as USB HID keyboard
    let driver = Driver::new(p.USBD, Irqs, HardwareVbusDetect::new(Irqs));

    let flash = Flash::take(mpsl, p.NVMC);

    // Aurora Sweep left-half pin map (input = cols, output = rows; row2col)
    ...
    let peripheral_addrs =
        read_peripheral_addresses::<1, _, ROW, COL, NUM_LAYER, 0>(&mut storage).await;
    ...
    join(
        join3(
            ...
            run_processor_chain! { EVENT_CHANNEL => [batt_proc], },
            keyboard.run(),
        ),
        join3(
